...
![ANEMO風場エディター。左側に風場名、地図操作、補間とファイルの設定があり、右側の地図に風の矢印が表示されている](../wind_generator/public/images/anemo-wind-field-editor.webp)

左側が操作・設定欄、右側が風を置く地図です。緑色の矢印は入力した風、青色の矢印は補間結果を示します。この画像は「例を読み込む」で説明用の合成データを表示した状態です。地図 © OpenStreetMap contributors
